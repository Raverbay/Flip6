(async function(){
  ...
  const wg=$('#worldGrid');
  cfg.worlds.forEach((w,i)=>{const a=document.createElement('a');a.className='world-card reveal';a.href='#brands';a.innerHTML=`<span class="world-number">${w.label}</span><div><h3>${w.name}</h3><p>${w.description}</p></div><span class="world-arrow">↗</span>`;wg.appendChild(a)});
